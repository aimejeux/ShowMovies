...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/TSmedia/lib/player/m3u8player/m3u8/model.py
# Compiled at: 2018-05-11 05:07:28
from collections import namedtuple
import os, errno, math, urlparse, re, parser
...
class M3U8(object):
    ...
    def _initialize_attributes(self):
        ...
        self.playlists = PlaylistList([ Playlist(base_uri=self.base_uri, **playlist) for playlist in self.data.get('playlists', [])
                                      ])
        return
...
class Segment(BasePathMixin):
    ...
    def __str__(self):
        output = [
         '#EXTINF:%s,' % int_or_float_to_string(self.duration)]
        if self.title:
...
class Playlist(BasePathMixin):
    ...
    def __init__(self, uri, stream_info, base_uri):
        ...
        if resolution != None:
            ...
            resolution_pair = None
        self.stream_info = StreamInfo(bandwidth=stream_info['bandwidth'], program_id=stream_info.get('program_id'), resolution=resolution_pair, codecs=stream_info.get('codecs'))
        return
...
def int_or_float_to_string(number):
    ...
    return str(number)
# okay decompiling F:\TSMstudio\bin\release\scripts\main\lib\m3u8player\m3u8\model.pyo
